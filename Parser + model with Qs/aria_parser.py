...
def parse_prot(content : str, res_info_dict : ResInfoDict) -> tuple[ChemShiftToAtom, AtomsByRes, ResIdToAA, dict()]:
    """
    ...
    Qs = [] #list of Qs from current resiude
    ...
    for line in lines : 
        ...
        if last_res_id != res_id:
            ...
            if AA_name == "CYS/SER": 
                res_id_to_AA[last_res_id] = "CYS/SER"
            else: 
        ...
        if "Q" == atom_type[0]:
            hydrogen_counter = 0
            if chem_shift != 999: Qs.append(atom_type)
        ...
        if chem_shift != 999: 
            chem_shift_to_atom[(chem_shift - err, chem_shift + err)] = atom_name
    AA_name = match_AA(seq, res_info_dict)
    if AA_name == "CYS/SER": res_id_to_AA[res_id] = "CYS/SER"        
    else: res_id_to_AA[res_id] = AA_name
    if AA_name != "XAA": 
        non_hydrogens = res_info_dict[AA_name][1]
        for a in non_hydrogens:
            atom_set[res_id].add(f'{a}_{res_id}')
    res_id_to_Qs[res_id] = Qs          
    return (chem_shift_to_atom, atom_set, res_id_to_AA, res_id_to_Qs)
...
def parse_peaks(content : str, chem_shift_to_atom : ChemShiftToAtom) -> NOEAssignment:
    ...
    for line in lines : 
        ...
        for a in Iq:
            if a in Ir and len(Ir) == 1 and len(Ip) == 1:
                Ir = []
...
def compute_Q(res_id_to_Q, res_id_to_seq, res_id_to_AA, res_info_dict) -> PairAssignment:
    
    Q_set = set() #set of atoms associated with a Q
    Q_dists = dict()

    for res_id, Qs in res_id_to_Q.items():
        AA_name = res_id_to_AA[res_id]

        #wait for reply from therese to treat this case
        #+ TRP we dont really know anything about
        if AA_name in "PHETYRTRP":
            continue
        
        seq = res_id_to_seq[res_id]
        seq = set(filter(lambda atom : atom[0] != "Q", seq))
        _, _, cov_lens, angle_lens = res_info_dict[AA_name]
        for Q in Qs:
            suffix = Q[1:]
            Q_set = set(a for a in seq if suffix in a)
            CN = next((a for a in Q_set if a[0] in "CN"), None)
            if CN == None:
                continue
            Q_set.remove(CN)
            if len(Q_set) == 2:
                a1, a2 = Q_set
                a1_name, _ = a1.split("_")
                a2_name, _ = a2.split("_")
                CN_name, _  = CN.split("_")
                Q_name = f'{Q}_{res_id}'
                s = angle_lens[tuple(sorted((a1_name, a2_name)))]
                l = cov_lens[tuple(sorted((a1_name, CN_name)))]

                qc = math.sqrt(4*l**2 - s**2)/3
                qh = math.sqrt(2*s**2+l**2)/3

                Q_dists[(a1, Q_name)] = qh
                Q_dists[(a2, Q_name)] = qh
                Q_dists[(CN, Q_name)] = qc
                ...
                qc = 3/4*math.sqrt(l**2 - s**2/3)
                qh = 3/4*math.sqrt(11/27*l**2+26/27*s**2)

                Q_dists[(a1, Q_name)] = qh
                Q_dists[(a2, Q_name)] = qh
                Q_dists[(a3, Q_name)] = qh
                Q_dists[(CN, Q_name)] = qc
    return Q_dists

def write_data(atoms: AtomsByRes, rhos: NOEAssignment,  cov_dists: PairAssignment, ang_dists : PairAssignment, q_dists: PairAssignment, filename = "NOE_data.dat", scale_factor = None):

    if scale_factor is None: 
        if len(rhos) != 0:
            scale_factor = mean(val[3] for val in rhos)*(3**6)
        else: 
            scale_factor = 0

    with open(filename, "w") as outfile:
        #write scale factor
        outfile.write(f'param scale := {scale_factor}; \n')
        #define atoms set
        ...
        #define Q-distance set
        outfile.write("set QDISTS := ")
        for a1, a2 in q_dists.keys(): outfile.write(f' {a1} {a2}')
        outfile.write(";\n")
        ...
        #give Q distance data
        for (a1, a2), dist in q_dists.items(): outfile.write(f' {a1} {a2} {dist}\n')
        outfile.write(";\n")
...
def write_initx(atoms_to_initial_coord, outfilename = "model/init_x.dat"):
